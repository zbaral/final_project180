...
int main(int argc, char** argv)
{
	...
	// scene.Add(&rect3);
	// scene.Add(std::make_unique<AreaLight>(Vector3f(213, 554, 227), 2, 130, 105));
	scene.Add(std::make_unique<Light>(Vector3f(278, 554, 279.5), 2));
	...
	Sphere sph(Vector3f(348, 83, 378), 83);
	// sph.m->m_type = REFLECTION_AND_REFRACTION;
	// sph.m->ior = 1.2;
	rect6.m->m_color = Vector3f(0.73, 0.73, 0.73);
	sph.m->Kd = 0.6;
	sph.m->Ks = 0;
	sph.m->specularExponent = 0;
	scene.Add(&sph);

	//box1
	// Box b1(Vector3f(265, 0, 295), Vector3f(430, 165, 460), 18);
	...
	//box2
	Box b2(Vector3f(130, 0, 65), Vector3f(295, 330, 230), -15);
	b2.m->m_color = Vector3f(0.73, 0.73, 0.73);
	b2.m->Kd = 0.6;
	b2.m->Ks = 0;
	b2.m->specularExponent = 0;
	scene.Add(&b2);
